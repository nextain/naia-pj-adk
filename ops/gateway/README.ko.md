...
`issue_lifecycle.py`는 네트워크와 독립된 Python 3 어댑터 부품입니다. 전체 게이트웨이를
대체하지 않습니다. `resolve_issue_key(text, items, repositories, thread_id)`에는
프로젝트 어댑터가 허용한 `owner/repository` 목록과 신뢰할 수 있는 추적표를 전달합니다.
기존 스레드 연결이 우선이고, 연결이 없으면 유일한 저장소 명시 참조를 사용합니다.
`#번호`만 있으면 기존 추적표에서 저장소가 하나로 정해질 때만 받습니다. 이름 검색이나
모델의 추측으로 연결을 만들지 않습니다. 새 이름은 `owner/repository#번호 요약`으로
만들고 자동 배분용 지시문을 이름에 사용하지 않습니다.

`may_dispatch`는 부모 채널·보조 기록·보관된 스레드·사람 확인 대기를 제외하는
자격 필터입니다. 권한 검사나 실행 중 작업 중복 검사를 대신하지 않으므로 기존 정책
검사와 함께 사용합니다. 스레드 편입과 생성은 명시적으로 확인된 이슈에만 수행합니다.
`is_conversation_message`를 통과한 실제 메시지에서 작성자를 판정해야 합니다.
스레드 이름 변경 같은 시스템 이벤트를 사람의 답변으로 세지 않습니다.

`project_execution(item, jobs)`는 어댑터가 읽어 온 작업 목록을 투영합니다. 각 작업은
`id`, `state`, 정규화된 UTC ISO `updatedAt`, `issueKey`, 선택적 `channelId`를 가집니다.
목록은 종료된 작업만 조회한 부분 목록이어서는 안 됩니다. 실행 중 작업이 있으면 그것이
우선입니다. 실행 완료는 `executionState=completed`이며 이슈 닫힘이나 전달 성공을
뜻하지 않습니다. 후속 상태는 `followup_pending`으로 전환하되 사람이 정리했거나
확인을 기다리는 상태는 보존합니다. `failed_with_alert`는 알림 필요 상태이며 알림이
실제로 전달됐다는 증거가 아닙니다. 알림 송신은 기존 알림 정책과 원장을 사용합니다.

추적표를 쓰는 **모든** 프로세스는 `update_tracker(path, transform)`으로 읽기부터
교체까지 같은 잠금을 사용해야 합니다. POSIX 로컬 파일시스템용이며 분산 잠금이
아닙니다. JSON 조회 실패와 변환 실패는 원본을 보존합니다. 실행 DB는 읽기만 합니다.
기존 v1 추적표의 `repo#번호` 읽기 호환은 유지하지만 새 함수는 저장소 소유자까지
확정된 키만 처리합니다. 실제 어댑터 도입 시 키와 중복 스레드 별칭을 명시적으로
마이그레이션하고, 전체 쓰기 경로가 잠금을 사용하는지 확인해야 합니다.

메타데이터 정리는 감사와 분리합니다. 이름·보관 여부·마지막 메시지의 변경 전 값을
기록하고, 수정 직전 다시 비교합니다. 보관된 스레드는 열기→이름 변경→재보관 순서로
처리하며, 요청 제한을 만나면 지정된 대기 후 변경 전 조건을 다시 확인합니다. 각 단계의
영수증을 남겨 실패 시 이번 작업이 바꾼 값만 되돌립니다. 메시지는 삭제하지 않습니다.
이는 어댑터 구현 계약이며 이 부품에 Discord API 수정 기능은 없습니다.

검증: `npm run test:issue-lifecycle`. 서로 다른 저장소의 같은 이슈 번호, 교차 참조,
시스템 이벤트, 실행/후속 상태, 이전 잘못된 키, 동시 쓰기와 실패 시 원본 보존을
오프라인으로 검사합니다. 실제 도입 완료는 해당 프로젝트의 연결·전달·상태 대조와
적용된 리비전 증거로 따로 확인해야 합니다. 되돌리기는 도입 전 추적표 백업과 이전
어댑터 리비전을 함께 사용하며, 새 기록을 덮어쓰지 않도록 쓰기를 정지하거나 비교합니다.
